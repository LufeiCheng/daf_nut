...
function checkCommandEnable_BlueDragonWill(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_BLUEDRAGONWILL); // ?????????? ???????? ????? ????????. ?????:?????? [2012.04.20] obj.sq_IsCommandEnable(SKILL_BROKENARROW);
	}
	...
	if(substate == SUB_STATE_BLUEDRAGONWILL_0) {
		...
		if(sq_var.get_vector(0)) { // ??????????? ????? ?? ???? ?????? ???????..
			if(sq_var.get_vector(1) != posY) { // ?? posY?? ??????? ????????..
				sq_var.set_vector(0, 0); // ????¡À???? off??????..
				sq_var.set_vector(1, posY);
			}
		}
		 
		if(obj.isMovablePos(dstX, dstY) && !sq_var.get_vector(0)) { // ????¡À???? ????????????? ??????????? ???
			sq_setCurrentAxisPos(obj, 0, dstX);
			sq_setCurrentAxisPos(obj, 1, dstY);
		}
		else { // ????? ?? ???? ?????? ??????..
			sq_var.set_vector(0,1); // ??? ?¡À??? ?¥å??? 3 ????? ?? ???? ?????? ?????? ?? ????? ????? ????????..
