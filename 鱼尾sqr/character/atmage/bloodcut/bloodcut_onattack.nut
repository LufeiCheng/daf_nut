...
//            CNSquirrelAppendage.sq_RemoveAppendage(damager, "character/mage/cirruscut/ap_cirruscut.nut");		
    ...
    if (id == 63)
    {
        ...
        {
            local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(damager, obj, SKILL_SHARDMAGNUM, false, "character/atmage/shardmagnum/ap_shardmagnum.nut", true);				 
...
//            addMonsterDeadPassiveObject(obj,damager);
    ...
    if (id == 22)
    {
        if (!CNSquirrelAppendage.sq_IsAppendAppendage(damager, "character/atmage/raginwind/ap_raginwind.nut"))
        {
            ...
            if (!sq_IsFixture(damager))
            {
                ...
                local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(damager, obj, 169, false, "character/atmage/raginwind/ap_raginwind.nut", true);				 
    ...
    if (id == 18)
    {
        ...
        if (state == 0 || state == 1)
        {
            ...
            {
                obj.getVar("dama").push_obj_vector(damager);
                local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(damager, obj, 169,
    ...
    if (id == 15)
    {
        ...
        if (obj.getVar("flag").get_vector(0) == 0)
        {
            if(damager.getState() != STATE_HOLD)
            {
                local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(damager, obj, 169, false, 
