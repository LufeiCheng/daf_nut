...
function proc_appendage_latentability(appendage)
{
	...
	local sqrChr = sq_GetCNRDObjectToSQRCharacter(obj);
	{
		...
		local value17 = sq_GetLevelData(sqrChr, SKILL_LATENTABILITY, 17, skillLevel); //?Û¯áÜÓø
		local value18 = sq_GetLevelData(sqrChr, SKILL_LATENTABILITY, 18, skillLevel); //ì¹?áÜÓø
		...
		if(change_appendage)
		{
			...
			change_appendage.addParameter(CHANGE_STATUS_TYPE_CAST_SPEED, true, value17.tofloat()); //?Û¯áÜÓø
			change_appendage.addParameter(CHANGE_STATUS_TYPE_MOVE_SPEED, true, value18.tofloat()); //ì¹?áÜÓø
